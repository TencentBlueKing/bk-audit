...
<script setup lang="ts">
      ...
      try {
        ...
        const log = Object.keys(jsonData).reduce((result, item) => {
          ...
          if (!_.isPlainObject(jsonData[item])) {
            if (_.isArray(jsonData[item])) {
              const arrayItem = handleArrayObject(jsonData[item]);
              jsonItem += `"${item}": [\n${arrayItem}\t],`;
            } else {
          ...
          const jsonObjItem = _.isPlainObject(jsonData[item]) ? `${jsonItem}` : `${jsonItem}`; // 对象注释拼在前面
  ...
  // 处理数组对象
  const handleArrayObject = (json: Array<any>) => json.reduce((res: string, arrayItem:any) => {
    const arrayJsonItem = handleJsonItem(arrayItem, '', true);
    ...
    const jsonObjItem = Object.keys(json).reduce((result, objItem) => {
      ...
      if (!_.isObject(json[objItem])) {
        ...
        if (_.isArray(json[objItem])) {
          const arrayItem = handleArrayObject(json[objItem]);
          jsonItem += `"${objItem}": [\n${arrayItem}    ],`;
        } else {
