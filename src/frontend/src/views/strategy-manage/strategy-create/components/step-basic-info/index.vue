          ...
          <card-part-vue :title="t('基础信息')">
            <template #content>
              <div class="basic-info-fields">
                <div class="basic-info-row flex-center">
                  <bk-form-item
...
<script setup lang="ts">
  ...
  import StrategyWaySelector from '../step1/components/strategy-way-selector.vue';
  ...
  const strategyWayList = ref<Array<ItemType>>([
    {
      label: t('自定义规则审计'),
      value: 'rule',
      config: {
  ...
  const strategyWayOptions = computed(() => strategyWayList.value.map(item => ({
    label: item.label,
    value: item.value,
  ...
  const updateFormData = (data: Record<string, any>) => {
    ...
    if (formData.value.configs.select?.length) {
      formRef.value.clearValidate('configs.select');
    }
  };
  ...
  const handleNext = () => {
    const doValidate = () => {
      ...
      setTimeout(() => {
        ...
        if (formData.value.strategy_type === 'model' && comRef.value?.getValue) {
...
.create-strategy-page {
  ...
  .scheme-fields {
    ...
    :deep(.strategy-customize.customize-basic-mode > .customize-rule > .bk-form-item),
    :deep(.strategy-customize.customize-basic-mode > .dispatch-wrap > .bk-form-item),
    :deep(.strategy-reference-model > .bk-form-item) {
      width: 100%;
    }

    :deep(.schedule-period-row .bk-form-item),
    :deep(.schedule-period-row .bk-form-content) {
      width: auto;
    }
  }
  ...
  .create-strategy-main {
    ...
    .strategt-form {
      ...
      :deep(.strategy-customize.customize-basic-mode) {
        ...
        .data-source-picker :deep(.dsp-trigger),
        .other-table-detail,
        .link-data-detail,
        .panel-edit {
          width: 100%;
