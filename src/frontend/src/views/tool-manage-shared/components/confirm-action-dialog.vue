...
<script setup lang="ts">
  ...
  const toolManageCtx = useToolManageContext();
  const { statusActionLabels } = toolManageCtx;
  ...
  const showDeleteInfoBox = (target?: ToolItem | null) => {
    ...
    const handleConfirm = () => {
      ...
      toolManageCtx.deleteTool(currentTarget.uid).then(() => {
  ...
  const showToggleStatusInfoBox = (target?: ToolItem | null, actionType?: ActionType) => {
    ...
    InfoBox({
      title: t(isEnabling ? statusActionLabels.enableTitle : statusActionLabels.disableTitle),
      subTitle: t(isEnabling ? statusActionLabels.enableSubTitle : statusActionLabels.disableSubTitle),
      confirmText: t(isEnabling ? statusActionLabels.enableConfirm : statusActionLabels.disableConfirm),
      cancelText: t('取消'),
      headerAlign: 'center',
      contentAlign: 'center',
      footerAlign: 'center',
      ...
      onConfirm() {
        return toolManageCtx.toggleToolStatus({
          uid: currentTarget.uid,
          enable: isEnabling,
        }).then(() => {
          messageSuccess(t(isEnabling
            ? statusActionLabels.enableSuccess
            : statusActionLabels.disableSuccess));
          emit('success');
        });
      },
    });
