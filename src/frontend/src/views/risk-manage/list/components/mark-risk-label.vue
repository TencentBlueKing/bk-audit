...
<template>
  <audit-popconfirm
    ...
    <template #contentHeader>
      <p>{{ t('风险ID') }}：<span style="color: #313238;">{{ data.risk_id }}</span> </p>
      {{ isMisreport
    ...
    <template #content>
      <audit-form
        ...
        <bk-form-item
          v-else-if="data.status !== 'auto_process'"
          class="is-required mr16"
          :label="t('处理人')"
    ...
    <div>
      <bk-button
        v-if="data.permission.edit_risk_v2 || data.current_operator.includes(userInfo.username)"
...
<script setup lang='ts'>
  ...
  const handleConfirm = () => formRef.value.validate().then(() => {
    ...
    if (!isMisreport.value && props.data.status === 'auto_process') {
      params.revoke_process =  isForce.value ;
    }
    if (!isMisreport.value) {
      params.description = formData.value.description;
    } else if (props.data.status !== 'auto_process') {
