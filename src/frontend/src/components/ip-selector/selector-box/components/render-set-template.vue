    ...
    <div class="render-set-template">
      ...
      <div class="topo-node-list">
        <bk-input
          ...
          :placeholder="t('搜索节点名')" />
        <bk-loading :loading="isNodeInstanceStatusLoading">
          <table>
            <thead>
              <tr>
...
<script setup lang="ts">
  ...
  interface Props {
    bizId: number,
    lastSetTemplateList: Array<TemplateTopoModel>
  }

  interface Emits {
    (e: 'change', type: 'setTemplate', value: Array<TemplateTopoModel>): void
  }

  const props = defineProps<Props>();
  const emits = defineEmits<Emits>();

  type TTemplateNodeStatusMap = Record<number, NodeInstanceStatusModel>
  type TNodeCheckedMap = Record<number, TemplateTopoModel>
  const { t } = useI18n();

  const nodeAgentStatusMap = shallowRef<TTemplateNodeStatusMap>({});
  const nodeCheckedMap = shallowRef<TNodeCheckedMap>({});

  let innerChange = false;

  // 同步外部变更
  watch(() => props.lastSetTemplateList, (lastSetTemplateList) => {
    if (innerChange) {
      innerChange = false;
      return;
    }
    nodeCheckedMap.value = lastSetTemplateList.reduce((result, item) => {
  ...
  // 获取 template topo node 的 agent status
  const {
    ...
    onSuccess(data) {
      nodeAgentStatusMap.value = data.reduce((result, item) => {
        ...
        result[item.bk_inst_id] = item;
        return result;
      }, {} as TTemplateNodeStatusMap);
    },
  });

  // 获取 template topo node 的基础信息
  const {
    loading: isNodeInstanceStatusLoading,
  ...
  const handleNodeCheckChange = (checked: boolean | string | number, node: TemplateTopoModel) => {
    ...
    const instIdList = Object.keys(nodeCheckedMap.value);
    fetchTemplateNodesBaseInfo({
      biz_id: props.bizId,
      bk_inst_ids: instIdList.join(','),
      template_type: 'SET_TEMPLATE',
    });

    innerChange = true;
    emits('change', 'setTemplate', Object.values(nodeCheckedMap.value));
  };
</script>
<style lang="postcss">
  .render-set-template {
