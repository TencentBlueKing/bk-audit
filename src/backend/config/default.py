...
# 意图识别智能体环境路由开关（值为 AIAgentCode 枚举名，默认专属智能体）：
...
# 常用操作高频化：滑动统计窗口（天）；桶 TTL = 窗口 + 2 天由代码推导，无需单独配置
AI_ASSISTANT_COMMON_QUERY_WINDOW_DAYS = int(os.getenv("BKAPP_AI_ASSISTANT_COMMON_QUERY_WINDOW_DAYS", 14))
