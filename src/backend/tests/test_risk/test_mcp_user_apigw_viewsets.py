...
from services.web.risk.resources.analyse_report import MCPListAnalyseReportRisk
...
class TestMCPUserRiskViewSet(SimpleTestCase):
    ...
    def test_report_risk_list_reuses_apigw_pagination_contract(self):
        route = MCPUserAnalyseReportViewSet.resource_routes[0]
        apigw_route = AnalyseReportAPIGWViewSet.resource_routes[0]

        self.assertEqual(route.resource_class, MCPListAnalyseReportRisk)
        self.assertFalse(route.enable_paginate)
        self.assertFalse(apigw_route.enable_paginate)
