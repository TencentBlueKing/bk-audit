...
新增返回 7 个可检索事件基本信息字段（固定顺序前置）。原有扩展字段返回行为保持不变：
裸字段名、set 去重、id 格式为 {display_name}:{field_name}。
"""
...
class TestListEventFieldsByStrategy(TestCase):
    ...
    def test_display_names_use_product_style(self):
        """基本字段 display_name 为产品指定文案（中文 + 英文字段名佐证）"""
        result = resource.risk.list_event_fields_by_strategy(strategy_ids=[self.strategy_1.strategy_id])
        display_map = {item["field_name"]: item["display_name"] for item in result[:7]}

        self.assertEqual(display_map["raw_event_id"], "原始事件ID(raw_event_id)")
        self.assertEqual(display_map["operator"], "责任人(operator)")
        self.assertEqual(display_map["event_time"], "事件时间(event_time)")
