...
class TestListRenderingInRiskTitle(TestCase):
    ...
    def test_simple_list_rendering(self):
        ...
        test_data = {
            "strategy_id": 1,
            "_title_template": "Risk: {{ event_data.list_field }} | {{ event_evidence.nested_list }}",
    ...
    def test_template_syntax_with_lists(self, mock_renderer_class):
        """测试模板语法（如过滤器）和模拟渲染逻辑"""
        title_template = "Count: {{ event_data.numbers|length }} | {{ event_data.numbers }}"

        test_data = {
            "strategy_id": 1,
            "_title_template": title_template,
        ...
        mock_renderer_instance.jinja_render.assert_called_once_with(
            title_template, {"event_data": {"numbers": "10, 20, 30"}, "event_evidence": {}, "strategy_id": 1}
        )
