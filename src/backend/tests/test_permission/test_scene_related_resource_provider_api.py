...
class TestSceneRelatedResourceProviderAPI:
    ...
    def test_rule_provider_list_with_non_scene_parent_returns_empty(self):
        """有 parent 但 parent 不是 scene 时应返回空结果"""
        RiskRule.objects.create(name="rule-with-risk-parent-1", scope={})
        RiskRule.objects.create(name="rule-with-risk-parent-2", scope={})
        provider = RuleResourceProvider()

        lr = provider.list_instance(
            FancyDict(parent=FancyDict(id="R1", type=ResourceEnum.RISK.id), search=None),
            Page(50, 0),
        )
