...
from apps.permission.handlers.backends import (
    get_iam_permission_backend,
    is_iam_v3_backend,
    is_iam_v4_backend,
)
...
class TestIAMPermissionBackends(SimpleTestCase):
    @override_settings(IAM_PERMISSION_BACKEND="v4")
    def test_v4_backend_helper(self):
        self.assertEqual(get_iam_permission_backend(), "v4")
        self.assertTrue(is_iam_v4_backend())
        self.assertFalse(is_iam_v3_backend())

    @override_settings(IAM_PERMISSION_BACKEND="v3")
    def test_v3_backend_helper(self):
        self.assertEqual(get_iam_permission_backend(), "v3")
        self.assertFalse(is_iam_v4_backend())
        self.assertTrue(is_iam_v3_backend())

    @override_settings(IAM_PERMISSION_BACKEND="unexpected")
    def test_unknown_backend_falls_back_to_v3(self):
        self.assertEqual(get_iam_permission_backend(), "v3")
        self.assertFalse(is_iam_v4_backend())
        self.assertTrue(is_iam_v3_backend())
...
class TestPermissionServiceSensitivePermissions(TestCase):
    @override_settings(IAM_PERMISSION_BACKEND="v4")
    @mock.patch("apps.permission.handlers.iam_v4.IAMV4Permission")
    def test_v4_sensitive_permissions_use_no_resource_action(self, mock_v4):
        mock_v4.return_value.is_allowed.return_value = True

        result = PermissionService(username="admin").get_sensitive_object_permissions(["so-1", "so-2"])

        self.assertEqual(result, {"so-1": True, "so-2": True})
        mock_v4.return_value.is_allowed.assert_called_once_with(
            ActionEnum.ACCESS_AUDIT_SENSITIVE_INFO,
            resources=[],
        )
        mock_v4.return_value.batch_is_allowed.assert_not_called()

    @override_settings(IAM_PERMISSION_BACKEND="v3")
    @mock.patch("apps.permission.handlers.permission.Permission")
    def test_v3_sensitive_permissions_use_sensitive_object_resources(self, mock_permission):
        mock_permission.return_value.batch_is_allowed.return_value = {
            "so-1": {ActionEnum.ACCESS_AUDIT_SENSITIVE_INFO.id: True},
            "so-2": {ActionEnum.ACCESS_AUDIT_SENSITIVE_INFO.id: False},
        }

        result = PermissionService(username="admin").get_sensitive_object_permissions(["so-1", "so-2"])

        self.assertEqual(result, {"so-1": True, "so-2": False})
        mock_permission.return_value.batch_is_allowed.assert_called_once()
        call_kwargs = mock_permission.return_value.batch_is_allowed.call_args.kwargs
        self.assertEqual(call_kwargs["actions"], [ActionEnum.ACCESS_AUDIT_SENSITIVE_INFO])
        self.assertEqual(
            [group[0].type for group in call_kwargs["resources"]], ["sensitive_object", "sensitive_object"]
        )
        self.assertEqual([str(group[0].id) for group in call_kwargs["resources"]], ["so-1", "so-2"])
