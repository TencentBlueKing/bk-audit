...
class FilterValueErrorTests(TestCase):
    ...
    def test_with_aggregation(self):
        ...
        with self.assertRaises(FilterValueError) as context:
            raise FilterValueError(field="created_by", value="admin", type="int", aggregation="count")

        self.assertEqual(str(context.exception), "条件表达式中字段 created_by 的值 count (admin) 无法转换成预期类型 int。")
