...
class TestSceneResource(TestCase):
    ...
    def test_scene_retrieve_risk_count_uses_default_recent_window(self):
        ...
        Risk.objects.create(
            raw_event_id="raw-scene-detail-closed",
            strategy=strategy,
    ...
    def test_scene_retrieve_risk_count_uses_request_time_window(self):
        """测试场景详情活跃风险数量支持按请求时间范围统计"""
        strategy = self._create_bound_strategy(self.scene, "详情风险请求时间窗口策略")
        recent_time = timezone.now()
        old_time = recent_time - relativedelta(months=6, hours=1)
        Risk.objects.create(
            raw_event_id="raw-scene-detail-request-window-recent",
            strategy=strategy,
            event_time=recent_time,
            event_end_time=recent_time,
        )
        Risk.objects.create(
            raw_event_id="raw-scene-detail-request-window-old",
            strategy=strategy,
            event_time=old_time,
            event_end_time=old_time,
        )
        Risk.objects.create(
            raw_event_id="raw-scene-detail-request-window-closed",
            strategy=strategy,
            display_status=RiskDisplayStatus.CLOSED,
            event_time=recent_time,
            event_end_time=recent_time,
        ...
        default_result = self.resource.scene.retrieve_scene({"scene_id": self.scene.scene_id})
        request_window_result = self.resource.scene.retrieve_scene(
            {
                "scene_id": self.scene.scene_id,
                "start_time": old_time - timedelta(seconds=1),
                "end_time": recent_time + timedelta(seconds=1),
            }
        )

        self.assertEqual(default_result["risk_count"], 1)
        self.assertEqual(request_window_result["risk_count"], 2)

    def test_get_scene_info_risk_count_uses_default_recent_window(self):
        """测试场景管理员详情风险数量默认仅统计近 6 个月活跃风险"""
        ...
        Risk.objects.create(
            raw_event_id="raw-scene-info-closed",
            strategy=strategy,
            display_status=RiskDisplayStatus.CLOSED,
            event_time=timezone.now(),
            event_end_time=timezone.now(),
    ...
    def test_get_scene_info_risk_count_uses_request_time_window(self):
        """测试场景管理员详情活跃风险数量支持按请求时间范围统计"""
        strategy = self._create_bound_strategy(self.scene, "管理员详情风险请求时间窗口策略")
        recent_time = timezone.now()
        old_time = recent_time - relativedelta(months=6, hours=1)
        Risk.objects.create(
            raw_event_id="raw-scene-info-request-window-recent",
            strategy=strategy,
            event_time=recent_time,
            event_end_time=recent_time,
        )
        Risk.objects.create(
            raw_event_id="raw-scene-info-request-window-old",
            strategy=strategy,
            event_time=old_time,
            event_end_time=old_time,
        )
        Risk.objects.create(
            raw_event_id="raw-scene-info-request-window-closed",
            strategy=strategy,
            display_status=RiskDisplayStatus.CLOSED,
            event_time=recent_time,
            event_end_time=recent_time,
        ...
        default_result = self.resource.scene.get_scene_info({"scene_id": self.scene.scene_id})
        request_window_result = self.resource.scene.get_scene_info(
            {
                "scene_id": self.scene.scene_id,
                "start_time": old_time - timedelta(seconds=1),
                "end_time": recent_time + timedelta(seconds=1),
...
class TestPlatformPanelSyncScenesAndSystems(TestCase):
    ...
    def test_scenes_and_systems_only_systems_bound(self):
        """测试只绑定系统的情况：不应该同步到任何场景（系统级可见，不通过场景分组展示）"""
        ...
        # 验证结果：只绑定系统时，不同步到任何场景（场景分组为空）
        group_items = SceneReportGroupItem.objects.filter(panel_id=str(self.panel.id))
        assert not group_items.exists()
