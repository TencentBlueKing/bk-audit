...
class TestSqlDataSearchExecutor(TestCase):
    ...
    def test_variable_missing(self):
        ...
        self.assertIn("变量A", str(cm.exception))
    ...
    def test_variable_format_error(self):
        ...
        with self.assertRaises(InvalidVariableFormatError) as cm:
            executor.execute(params)
        self.assertIn("格式无效", str(cm.exception))
    ...
    def test_variable_parse_fallback_error(self):
        ...
        executor = SqlDataSearchExecutor(source=config, analyzer_cls=self.analyzer_cls)
        # mock _format_input 抛出非 InputVariableValueError
        import types

        def broken_func(_):
            raise RuntimeError("mock error")

        origin_func = VariableValueParser._format_input
        executor.render_value = types.MethodType(
            lambda self, var, value: VariableValueParser(var).parse(value), executor
        )
        # mock _format_input
        VariableValueParser._format_input = broken_func
        ...
        with self.assertRaises(ParseVariableError) as cm:
            executor.execute(params)
        self.assertEqual(cm.exception.message, '解析变量类型:输入框;值:xxx 异常,请联系系统管理员')
        # 恢复原始函数
        VariableValueParser._format_input = origin_func
    ...
    def test_sql_execute_time_range_select_type(self):
        ...
        expected = {
            ...
            f"FROM (SELECT * FROM table WHERE a IN ({ts_start}, {ts_end})) AS _sub",
            'num_pages': 10,
            'page': 1,
            'query_sql': f"SELECT * FROM table WHERE a IN ({ts_start}, {ts_end}) LIMIT 10",
            'results': [{'field1': 'value1'}, {'field2': 'value2'}],
            'total': 2,
        }
        self.assertDictEqual(expected, result)
