...
class TestIAMV4SceneRoleMembers(SimpleTestCase):
    ...
    def test_refresh_scene_members_skips_empty_v4_result_for_existing_local_members(self, _mock_members, _mock_backend):
        scene = mock.Mock()
        scene.scene_id = 1
        scene.managers = ["admin"]
        scene.users = ["viewer"]
        ...
        self.assertEqual(result, {"managers": ["admin"], "users": ["viewer"]})
        scene.save.assert_not_called()
