...
class AIAuditAnalyse(BkApiResource, abc.ABC):
    """AI审计分析智能体API基类"""

    module_name = "bk_plugins_ai_audit_analyse"
    base_url = AI_AUDIT_ANALYSE_API_URL
    platform_authorization = True
    tags = ["AIAuditAnalyse"]
    TIMEOUT = 600  # 分析报告生成耗时较长

    @property
    def app_code(self) -> str:
        """获取 APP_CODE，优先使用自定义配置"""
        return settings.AI_AUDIT_ANALYSE_APP_CODE or settings.APP_CODE

    @property
    def secret_key(self) -> str:
        """获取 SECRET_KEY，优先使用自定义配置"""
        return settings.AI_AUDIT_ANALYSE_SECRET_KEY or settings.SECRET_KEY
...
class ChatCompletion(AIAuditAnalyse):
    """分析智能体对话接口（应用态）
    ...
    def _parse_stream_response(self, response) -> str:
        ...
        if done_content is not None:
            logger.info("AI Analyse stream done content: %s", done_content)
        logger.info("AI Analyse stream text content: %s", text_content)
        return text_content
