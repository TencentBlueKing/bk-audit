...
# 默认值覆盖的值类型：支持 str/int/float/bool/dict/list/None
OverrideValue = Union[str, int, float, bool, dict, list, None]
...
class ScopeDefaultValueOverride(BaseModel):
    ...
    overrides: Dict[str, OverrideValue] = Field(
        ...
        description="当前 scope 命中的默认值覆盖映射，key 为变量 uid，value 为默认值（支持 str/int/float/bool/dict/list/None）",
    )
