...
class UpdatePlatformPanel(BKVision):
    ...
    def perform_request(self, validated_request_data):
        ...
        with transaction.atomic():
            for field in ["name", "category", "description", "status", "vision_id", "default_value_overrides"]:
...
class GetPanelDetail(BKVision):
    ...
    def perform_request(self, validated_request_data):
        ...
        if not isinstance(default_value_overrides, dict):
            ...
            raise ValidationError({"default_value_overrides": "格式错误，必须为对象类型"})

        # 使用 ScopeContext 统一处理 scope 相关逻辑
        scope = ScopeContext(
            scope_type=validated_request_data["scope_type"],
            scope_id=validated_request_data.get("scope_id"),
        )
        ...
        if scope.is_scene_scope and scope.scope_id:
            ...
            default_value_override = scenes_overrides.get(str(scope.scope_id)) or default_value_overrides.get(
                "default", {}
            )
            ...
            default_value_override = systems_overrides.get(str(scope.scope_id)) or default_value_overrides.get(
                "default", {}
            )
        else:
