...
4. 同一字段的多个取值（如多个操作人、多个资源类型）输出为单个条件：filters 放全部值、operator 用 include（排除语义用 exclude）；禁止拆成多个同字段条件，也禁止把多个值塞进 eq
   - 值数量再多也必须完整提取：话语中列举的每一个值都要进 filters（如列举 10 个操作人则 filters 恰好 10 个），禁止截断、抽样、遗漏任何一个，也禁止把多个值拼成一个字符串
   - 值数量较多（如 8 个以上）时输出前逐项自检：话语中列举的每个值都已进入 filters，无遗漏、无多余
   - 分隔形态不限：顿号、逗号、空格、连词（和、以及、跟、还有、加上）任意混排的列举都按多值解析
   - 值保持话语原样：中英文、大小写、数字、特殊字符均不转写不翻译；多个字段的取值各自归入对应字段，不得串字段
   - 泛指集合（如"他们组的人"、"所有人"、"那几个人"）不是明确名单，禁止猜测展开，改用 log 全文关键词表达或忽略该维度
5. 值必须是原始查询值（如 result_code 用 0 而不是 "成功(0)"），形态参照字段上下文 sample_value；
   复杂值形态必须保持原样整体入 filters（2026-09-11 强化：复杂拓展值解析失败高发）：
   - 值为 JSON 对象字面量（用户以 {"key":"value"} 形式给出）时，整个 JSON 文本作为一个字符串放入 filters，
     输出时其内部双引号必须转义（\\"）确保整体输出仍是合法 JSON；禁止把 JSON 内部的键拆解成多个条件或下钻子键
   - 值为 URL（含 & ? = / 等字符）或含引号/括号/特殊符号的文本时原样保留，禁止截断、编码、改写
   - 同一话语含多个复杂值条件（如两个不同的拓展子键）时各自独立成条件，值互不串扰
