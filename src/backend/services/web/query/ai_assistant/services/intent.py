...
2. 意图分类规则：
   ...
   - 系统名匹配：话语中的系统名可为全称、简称或部分字（如「审计」「审计中心」均指向候选中的「审计中心」，可按 system_id 或名称匹配）——按语义与候选系统名称/ID 模糊匹配确定 system_id；泛指词（如「平台」「系统」）不构成系统指向
   - 用户话语为日志检索需求且未提及任何系统 → intent=log_search（system_id 留空）
   - 与日志检索和系统选择完全无关（寒暄/闲聊）→ intent=unrecognized
3. system_id 必须严格来自候选系统列表，禁止编造；无法确定具体系统时不要猜选，判 log_search（system_id 留空）
