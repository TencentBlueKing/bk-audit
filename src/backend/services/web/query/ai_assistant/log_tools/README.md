...
MCP 默认 TopN 10，程序统计默认 100，协议最大 500，部署配置可收紧；OTHER/MISSING 不占 TopN 名额。默认最多 1440 个时间桶、100000 个数值单元和 4 MiB 业务结果。单元预算结合组数、桶数和指标列计算；程序统计每组每桶只有计数列，MCP 还需考虑辅助计数/比例列。
