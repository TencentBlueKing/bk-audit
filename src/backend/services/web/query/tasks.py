...
def send_alert_for_failed_task(task: LogExportTask, error_msg: str):
    """
    对重试次数超过或等于最大重试次数的失败任务进行告警（预警）
    """
    timestamp = int(task.updated_at.timestamp()) * 1000
        ...
        "data": [
            {
                ...
                "event": {"content": f"任务 {task.id} 连续失败超过 {task.repeat_times} 次，错误信息: {error_msg}"},
                "target": "bk_audit",
                "dimension": {"module": "log_export", "location": "process_one_log_export_task"},
                "timestamp": timestamp,
            }
        ],
    }
    try:
        ...
        logger_celery.error(f"失败任务告警上报失败，任务 ID={task.id}, 错误: {alert_error}")
...
def alert_failed_export_tasks():
    """
    扫描失败次数超过阈值的任务进行告警（避免在异常中直接告警）
    """
    max_retries = settings.PROCESS_LOG_EXPORT_TASK_MAX_REPEAT_TIMES
    tasks = LogExportTask.objects.filter(status=TaskEnum.FAILURE.value, repeat_times__gte=max_retries, alert_sent=False)

    for task in tasks:
        try:
            send_alert_for_failed_task(task, "任务失败")
            task.alert_sent = True
            task.save(update_fields=["alert_sent"])
            logger_celery.info(f"告警已发送，任务 ID={task.id}")
        except Exception as e:
            logger_celery.error(f"任务告警发送失败，任务 ID={task.id}, 错误: {str(e)}")
