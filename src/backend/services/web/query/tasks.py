...
def process_log_export_task(task_id: int = None):
    ...
    start_time = datetime.now() - timedelta(seconds=settings.LOG_EXPORT_TASK_MAX_PERIODIC_TIME)
    tasks: QuerySet[LogExportTask] = LogExportTask.objects.filter(updated_at__lte=start_time,).filter(
