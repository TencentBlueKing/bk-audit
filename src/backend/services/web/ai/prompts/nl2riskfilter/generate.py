...
1. **action_input 必须是纯 JSON 对象**，只含用户明确提及的筛选字段，不需要的字段不传。所有字符串值和数组元素必须使用双引号（JSON 标准），禁止使用单引号。最终回答只能输出裸 JSON 对象本身，禁止使用 Markdown 代码块（如 ```json）、禁止添加解释性文字、前后缀、注释或自然语言说明
