...
class DeleteScene(SceneResource):
    ...
    def perform_request(self, validated_request_data):
        ...
        has_resources = (
            VisionPanel.objects.filter(scene_id=scene.scene_id).exists()
            or Tool.objects.filter(is_deleted=False, scene_id=scene.scene_id).exclude(scope_type="").exists()
            or Strategy.objects.filter(scene_id=scene.scene_id).exists()
            or NoticeGroup.objects.filter(scene_id=scene.scene_id).exists()
            or ProcessApplication.objects.filter(scene_id=scene.scene_id).exists()
            or RiskRule.objects.filter(scene_id=scene.scene_id).exists()
        )
        if has_resources:
