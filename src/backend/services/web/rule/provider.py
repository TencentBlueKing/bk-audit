...
from services.web.risk.models import RiskRule
from services.web.scene.constants import ResourceVisibilityType
from services.web.scene.models import ResourceBindingScene


class RuleResourceProvider(IAMResourceProvider):
    resource_type = "rule"
    ...
    def list_instance_by_policy(self, filters, page, **options):
        ...
        queryset = RiskRule.objects.filter(django_filters).order_by("id")
        results = [
    ...
    def filter_list_instance_results(
        ...
        qs = RiskRule.objects.all().order_by("id")
        if parent_id and resource_type == ResourceEnum.SCENE.id:
            ...
            qs = RiskRule.objects.filter(id__in=bound_ids).order_by("id")
    ...
    def filter_fetch_instance_results(self, ids: List[str]) -> Tuple[List[dict], int]:
        qs = RiskRule.objects.filter(id__in=ids)
    ...
    def filter_search_instance_results(
        ...
        qs = RiskRule.objects.filter(name__icontains=keyword).order_by("id")
        if parent_id and resource_type == ResourceEnum.SCENE.id:
            bound_ids = ResourceBindingScene.objects.filter(
                binding__resource_type=ResourceVisibilityType.RISK_RULE,
                scene_id=parent_id,
