...
> 💡 **提示**：Admin 后台支持**界面化配置**筛选条件，无需手写 JSON。点击「条件」字段旁的编辑按钮即可进入可视化配置界面。

#### 界面化配置方式

1. 在配置项的「条件」字段，点击 **编辑** 按钮
2. 在弹出的界面中选择：
   - **字段**：要筛选的字段（如 `system_id`、`action_id`）
   - **操作符**：比较方式（如 等于、包含、在列表中）
   - **值**：筛选值
3. 可添加多个条件，多个条件之间是 **AND** 关系
4. 点击 **确定** 保存
...
| 需求 | 字段 | 操作符 | 值 |
|------|------|--------|-----|
| 只要配置平台的日志 | `system_id` | 等于 | `bk_cmdb` |
| 只要多个系统的日志 | `system_id` | 在列表中 | `bk_cmdb, bk_job, bk_monitor` |
| 只要成功的操作 | `result_code` | 等于 | `0` |
| 只要编辑主机的操作 | `action_id` | 等于 | `edit_host` |
...
这是一个**多对多**的关系，理解这个关系对正确配置订阅非常重要。

#### 场景 1：一个配置项关联多个数据源

```
配置项 A（条件：system_id = 'bk_cmdb'）
  ├── 数据源 1：审计日志主表
  └── 数据源 2：审计日志归档表
```

**效果**：**同一个筛选条件**会应用到**所有关联的数据源**。适用于：
- 多个数据源字段相同，条件相同，避免重复配置

#### 场景 2：多个配置项关联同一个数据源

```
配置项 A（条件：system_id = 'bk_cmdb'） ─┬─→ 数据源：审计日志主表
配置项 B（条件：system_id = 'bk_job'）  ─┘
```

**效果**：多个配置项的条件用 **OR** 连接，生成的 SQL 为：

```sql
WHERE (system_id = 'bk_cmdb') OR (system_id = 'bk_job')
```

**适用于**：需要从同一数据源获取多种类型日志（满足任一条件即可）

#### 场景 3：多个配置项关联不同数据源

```
配置项 A（条件：system_id = 'bk_cmdb'） ──→ 数据源 1：配置平台日志表
配置项 B（条件：system_id = 'bk_job'）  ──→ 数据源 2：作业平台日志表
```

**效果**：各自独立，互不影响。查询时按 `source_id` 参数指定的数据源执行对应的筛选条件。

#### 💡 配置建议

| 场景 | 推荐配置方式 |
|------|-------------|
| 日志分布在多个表 | 一个配置项关联多个数据源 |
| 需要多种筛选条件 | 多个配置项关联同一数据源 |
| 不同系统日志分开存储 | 各配置项关联各自的数据源 |
