...
class CreateTool(ToolBase):
    ...
    def _create_sql_tool(self, config: dict):
        try:
            ...
            raise ValidationError({"config": f"SQL 配置不合法: {str(e)}"})
    ...
    def _create_tool_instance(self, tool_type: str, config: dict, name: str, custom_uid: str = None) -> Tool:
        ...
        tool = Tool.objects.create(
            ...
            auth_type=config.get("auth_type", "specify_user"),
            tool_type=tool_type,
            config=config,
        )
        ...
        if auth_users:
            tool.auth_users.set(auth_users)
...
class UpdateTool(ToolBase):
    ...
    def _update_sql_tool(self, tool: Tool, config: dict):
        try:
            ...
            raise ValidationError(f"SQL 配置不合法: {e}")
...
class ListToolAll(ToolBase):
    ...
    def perform_request(self, validated_request_data):
        tool_queryset = Tool.objects.filter(is_deleted=False).prefetch_related("auth_users")
...
class GetToolDetail(ToolBase):
    ...
    def perform_request(self, validated_request_data):
        ...
        data = {
            ...
            "auth_users": list(tool.auth_users.values_list("username", flat=True)),
            "config": tool.config,
            "panel": None,
        }
