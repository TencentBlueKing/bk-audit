...
class InvalidVariableFormatError(InputVariableValueError):
    ...
    MESSAGE = gettext_lazy("变量 {var_type} 的值格式无效: '{value}'")
...
class InvalidVariableStructureError(InputVariableValueError):
    ...
    MESSAGE = gettext_lazy("变量 {var_type} 的值结构无效: 期望 {expected_structure},但收到的是 {actual_type}")
...
class ParseVariableError(ToolException):
    ...
    STATUS_CODE = 500
    ERROR_CODE = "006"
    MESSAGE = gettext_lazy("解析变量类型:{var_type};值:{value} 异常,请联系系统管理员")
