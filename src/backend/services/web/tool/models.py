...
class AuthUser(models.Model):
    ...
    username = models.CharField(gettext_lazy("用户名"), max_length=64, db_index=True, null=True, blank=True)

    class Meta:
        verbose_name = gettext_lazy("Auth User")
...
class Tool(SoftDeleteModel):
    ...
    auth_users = models.ManyToManyField(
        AuthUser, verbose_name=gettext_lazy("授权用户"), blank=True, related_name="authorized_tools"
    )
