...
class Tool(SoftDeleteModel):
    ...
    auth_type = models.CharField(gettext_lazy("权限类型"), choices=PermissionTypeEnum.choices, max_length=16)
    auth_users = models.JSONField(gettext_lazy("授权用户"), default=list, blank=True, help_text=gettext_lazy("用户rtx列表"))
    auth_user_groups = models.JSONField(
        gettext_lazy("授权用户组"), null=True, blank=True, default=list, help_text=gettext_lazy("用户组ID列表")
    )
